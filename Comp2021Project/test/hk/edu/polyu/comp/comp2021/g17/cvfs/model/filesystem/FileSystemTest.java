...
*/
	...
	void newDiskTest(){
		try {
			//assertThrows(InvalidArgumentException.class, () -> fileSystem.newDisk("asdasd"));// ？
			...
			assertThrows(InvalidArgumentException.class, () -> fileSystem.newDisk("40")); // size <=40
		}
		catch (Exception e){
			e.printStackTrace();
		}
	...
	void newDirTest(){
		try {
			...
			fileSystem.newDisk("130"); // maxSize = 130
			fileSystem.newDir(name);
			assertThrows(InvalidFileNameException.class, () -> fileSystem.newDir("."));//current size = 120
	...
	void deleteTest(){
		try{
			...
			assertThrows(FileNotExistException.class, ()->fileSystem.currentDisk.getcwd().findDoc("PigWeight"));
			assertEquals(fileSystem.currentDisk.getcwd().getSize(),40+40);
			fileSystem.delete("Pig");
			assertThrows(FileNotExistException.class, ()->fileSystem.currentDisk.getcwd().findDir("Pig"));
			assertEquals(fileSystem.currentDisk.getcwd().getSize(),40);
			assertThrows(IllegalOperationException.class, ()->fileSystem.delete("."));
		}
		catch (Exception e){
			e.printStackTrace();
		}
	...
	void renameTest(){
		try{
			...
			assertThrows(FileNotExistException.class, ()->fileSystem.rename("Pi FatPig"));
	...
	void changeDirTest(){
		try{
			assertThrows(IllegalOperationException.class, ()->fileSystem.changeDir(".."));
	...
	void newSimpleCriTest(){
		try{
			...
			fileSystem.newSimpleCri("sn size G 40");
			...
			assertTrue(fileSystem.criteria.get("cn").assertCri(fileSystem.currentDisk.getRoot())); // ? Pig should be contained in Root
			System.setOut(Oriout);
		}
		catch (Exception e){
			e.printStackTrace();
	...
	void newBinaryCriTest(){
		try{
			...
			System.setOut(new PrintStream(testOut));
			fileSystem.newSimpleCri("cn name contains PigWeight ");
			fileSystem.newSimpleCri("sn size LE 50");
			assertThrows(InvalidArgumentException.class, ()->fileSystem.newBinaryCri("Tn cn && sp"));
			fileSystem.newBinaryCri("TN cn && sp"); // ? wrong design input in filesystem
			assertTrue(fileSystem.criteria.containsKey("TN"));
			String expectedOut = "             cn |       name |    contains |            PigWeight";
	...
	void searchTest(){
		try{
			...
			fileSystem.newNegation("nc cn");
			fileSystem.newSimpleCri("tc type equals txt");
			assertThrows(InvalidArgumentException.class, ()->fileSystem.search("pi"));
			//fileSystem.newBinaryCri("TN cn && sp"); // ? wrong design input in filesystem
	...
	void rSearchTest(){
		try{
			...
			fileSystem.newNegation("nc cn");
			fileSystem.newSimpleCri("tc type equals txt");
			assertThrows(InvalidArgumentException.class, ()->fileSystem.rSearch("pi"));
			//fileSystem.newBinaryCri("TN cn && sp"); // ? wrong design input in filesystem
	...
	void storeTest(){
		try{
			assertThrows(UsageException.class, ()->fileSystem.store("081707")); // not catch
			fileSystem.store("C:\\toStore");
	...
	void loadTest(){
		try{
			assertThrows(UsageException.class, ()->fileSystem.load("081707")); // not catch
			fileSystem.store("C:\\toStore");
			fileSystem.load("C:\\toLoad");
